...
const PureSearchMemoryTool = ({ tool }: ToolProps) => {
  if (!tool.input) {
    return null;
  }
  ...
  if (!isLoading) {
    ...
    return (
      ...
      //   <div>{hasError ? `Failed to search memories` : `Searched memories`}</div>
      // </div>
      null
    );
  }
  return (
    <div className='my-1 flex w-fit items-center gap-1 rounded-md text-base'>
      ...
      <ShinyText text={`Recalling what I know...`} />
    </div>
  );
};

const SearchMemoryTool = memo(PureSearchMemoryTool, (prevProps, nextProps) => {
